...
# Plot the 1. signal
ax[0].plot(data, color=colors[1], alpha=0.8)
ax[0].set_xlabel('Sample')
ax[0].set_ylabel('Amplitude [arb. u.]')
ax[0].set_title(f"Raw Signal: {data_path.split('/')[-1]}")

# Plot the 2. signal
ax[1].plot(data2, color=colors[3], alpha=0.8)
ax[1].set_xlabel('Sample')
ax[1].set_ylabel('Amplitude [arb. u.]')
ax[1].set_title(f"Raw Signal: {data_path2.split('/')[-1]}")

# Plot both spectra
ax[2].plot(freq, np.abs(bare_spectrum), label=f"{data_path.split('/')[-1]}", color=colors[1], alpha=0.8, lw=1.5)
ax[2].plot(freq, np.abs(bare_spectrum2), label=f"{data_path2.split('/')[-1]}", color=colors[3], alpha=0.8, lw=1.5)
