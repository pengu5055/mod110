"""
Use Wiener filter and deconvolve the image lena with kernel k2
"""
import numpy as np
...
deconvolved_images = {}
for noise, image in lena_k1.items():
    deconvolved_images[noise] = restoration.wiener(image, kernel1, 1e7)
...
for noise, image in windowed_images.items():
    ...
    windowed_deconvolved[noise] = windowed_deconvolved[noise] / np.max(windowed_deconvolved[noise]) * np.pi
    #windowed_deconvolved[noise] = restoration.unwrap_phase(windowed_deconvolved[noise])
...
for i, (noise, image) in enumerate(windowed_deconvolved.items()):
    ...
    axs[2, i].set_title(f'w/ Gaussian Window')

plt.savefig("./ImageDeconvolution/Images/lena_k2_deconvolved.png", dpi=500)
