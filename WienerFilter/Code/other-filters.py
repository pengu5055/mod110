"""
Plot similar style mega plot with the use of some other filters.
"""
import numpy as np
...
filtered = {
    'Wiener': signal.wiener(signal1, mysize=5),
    'Chebyshev Type II': signal.sosfiltfilt(signal.cheby2(5, 40, 0.5, 'low', output='sos'), signal1),
    'Median': signal.medfilt(signal1, 5),
    'Savitzky-Golay': signal.savgol_filter(signal1, 5, 1),
    'Butterworth': signal.sosfiltfilt(signal.butter(5, 0.5, 'low', output='sos'), signal1),
    'Downsample after AA': signal.decimate(signal1, 1),
    'Convolve w/ uniform': signal.convolve(signal1, np.ones(5)/5, mode='same'),
    'Bessel/Thompson': signal.sosfiltfilt(signal.bessel(5, 0.5, 'low', output='sos'), signal1),
}
...
ax_u[0].text(0.5, 0.3, f'Different Filters Applied\n to {data_path.split("/")[-1]}', fontsize=16, ha='center',
...
for i, (key, value) in enumerate(filtered.items()):
    idx = 3*i+1
    ax[idx].sharex(ax[22])
    # ax[idx].axhline(np.mean(np.abs(signal0) - np.abs(filtered["Wiener"])), color='black', linestyle='--')
    if key == "order1":
        ax[idx].plot(np.abs(np.abs(signal0)[:-1] - np.abs(value)), color=colors[i])
    else:
...
for i, (key, value) in enumerate(filtered.items()):
    ...
    ax[idx].sharex(ax[23])
    if key == "order1":
        ax[idx].plot(freq[:-1], np.abs(fft.fft(value)[:len(value)//2]), color=colors[i])
    else:
...
ax[0].set_title('Filtered Signals')
...
plt.savefig(f"./WienerFilter/Images/filters-{data_path.split('/')[-1]}.png", dpi=700)
plt.show()
